...
public class CNFConverter {
	...
	public String[] convertToCNF(String[] statements) {		
		...
		System.out.println(predicateMap.keySet());
		System.out.println(predicateMap.get("A"));
		return statements;		
	}
	...
	private String findPredicates(String string) {
		...
		for ( int i = 0; i < string.length(); i++){
			if ( string.charAt(i) >= 65 && string.charAt(i) <= 90){
				...
				string = string.substring(0, openIndex)+additions+string.substring(j+1);//只有一项的话后面就没有了P()|Q() P() 
				i = openIndex;
			}
		}
		mapList.add(map);
						...
						if (string.charAt(rightIndex) == ',' || string.charAt(rightIndex) == ')'){
							if (isVariable){
								...
								if (string.charAt(len+1) >= 97 && string.charAt(len+1) <= 122){
									...
									//string = string.substring(0, rightIndex)+string.substring(rightIndex);
